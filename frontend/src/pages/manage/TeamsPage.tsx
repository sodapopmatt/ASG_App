...
export default function TeamsPage() {
  ...
  const { data: companies = [], isLoading: loadingCompanies } = useQuery({ queryKey: ['companies'], queryFn: getCompanies, staleTime: Infinity })
  ...
  const isLoading = view === 'companies' ? loadingCompanies : loadingSports
        ...
        {(['companies', 'sports'] as View[]).map(v => (
          <button
            ...
            className={`flex-1 py-1.5 text-sm font-semibold rounded-md transition-colors ${
              view === v ? 'bg-white text-slate-800 shadow-sm' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
        ...
        {isLoading ? (
          ...
          filteredSports.length === 0 ? (
            ...
            filteredSports.map(sport => (
              <button
                key={sport.id}
                onClick={() => navigate(`/manage/teams/sport/${sport.id}`)}
                className="w-full flex items-center gap-3 px-4 py-3 bg-white rounded-xl border border-gray-200 shadow-sm text-left active:bg-gray-50 transition-colors"
              >
                <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center shrink-0">
