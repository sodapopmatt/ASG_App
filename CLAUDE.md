...
- `scoring_mode` (TEXT) — `placement` (default; admin awards via `/event-points/award-placement`) | `donation_count` (admin enters per-company item counts via `/donation-counts`; standings NOT live — an admin reviews the bucket-derived placements on ScoringPage, top=15/second=10/≥10 items=5/else=0, and explicitly saves via `/event-points/award-placement`; `POST /donation-counts/sports/{id}/recompute` remains available for a bulk recompute) | `water_ball_toss` (real per-team matches, `bracket_type='heats'`; rounds survived entered via the generic heat-result endpoint; a company's score is the best of its own teams; standings are NOT live — an admin reviews computed placements on ScoringPage and explicitly saves via `POST /waterball-results/sports/{id}/recompute`, ties share points) | `executive_golf` (real per-team matches, `bracket_type='heats'`, in two heat brackets "Round 1"/"Round 2"; 3 hole scores per company entered via `/golf-results` — stored as a JSON array in `matches.notes`, total in `matches.home_score`; ranked by Round-2 total, lowest wins; standings NOT live — an admin reviews and saves via `POST /golf-results/sports/{id}/recompute`, ties share points)
...
Per-company donation totals for donation-style sports (Canned Food Drive). **Not live**: writes to `/donation-counts` (`PUT`/`DELETE`) no longer touch `event_points` — entering a count only updates `donation_counts`, same as Water Ball Toss/Golf. Standings are reviewed and saved from ScoringPage's `DonationScoringSection`, which mirrors the bucket rules client-side as an editable preview (top distinct count=15, second distinct count=10, any remaining count ≥10=5, else 0) with placement AND points independently overridable per row, saving via the generic `POST /event-points/award-placement` — same mechanism as every other computed sport. `POST /donation-counts/sports/{id}/recompute` (`backend/app/routers/donation_counts.py`) still exists and bulk-rebuilds `event_points` the same way in one call (synthetic placement 1/2/3 per bucket), but the frontend no longer calls it automatically — `DonationResultsPage`'s "Reset All" instead clears counts and calls the generic `DELETE /event-points` directly.
...
- **Scoring** (ScoringPage): reuses the same flat-heats ranking as Human Pyramid — `rankFlatHeats(matches, 'high_wins', ...)` in `frontend/src/lib/ranking.ts` ranks teams by rounds survived (ties grouped, forfeits last with 0 points), then `collapseToCompanies` keeps each company's best-placing team (not an average — refs only track whichever team went furthest) and compresses placements to 1..N, tied companies sharing averaged points. Rendered in the same `AutoRankedScoringSection` every other computed sport uses (per-row Edit unlock, placement AND points editable, Publish Standings/Clear Points), saving per company via the generic `POST /event-points/award-placement` — same mechanism as Relay Race/Golf/Pickleball. `backend/app/routers/waterball_results.py`'s `POST /waterball-results/sports/{id}/recompute` (best-of-company score ranked, placement scale awarded, ties averaged) still exists and computes the same result in one bulk call, but the frontend no longer calls it now that placements are editable per row — same as Executive Golf's bulk recompute endpoint.
- **Not live**: entering a result on `WaterballResultsPage` does NOT touch `event_points` — it only updates the match. Standings only update when an admin opens ScoringPage's Water Ball Toss section and taps **Publish Standings**.
...
| `heats` | Yes — flat (one entry per team) OR grouped multi-phase (one bracket per heat) | Full for Relay Race (multi-phase: prelims → semis → final); flat for Human Pyramid and Water Ball Toss (`scoring_mode='water_ball_toss'` — grouped into "Group A"/"Group B" for organization only; scoring reuses the generic flat-heats ranking/ScoringPage flow, same as Human Pyramid); grouped two-round ("Round 1"/"Round 2") for Executive Golf (`scoring_mode='executive_golf'` — bespoke 3-hole totals + lowest-wins Round-2 ranking) |
...
- `GET /sports/{id}/standings` computes W-L per pool from terminal matches (completed/forfeit → winner W / opponent L; double_forfeit → both L); rank by wins desc, then an automatic tiebreak chain (`_rank_standings` in `sports.py`): head-to-head (only for a straight 2-team tie — a 3+-way tie can't be resolved by one match) → point differential desc → total points scored desc (point_diff/total_points are summed from `home_points_total`/`away_points_total`, currently populated by Pickleball only — other pool_bracket sports have these at 0 and so fall straight through to a shared rank, same as before). Anything still tied after all of that shares a rank — the admin resolves it manually when seeding the bracket phase (the seed list is drag-to-reorder before confirming, so this default order is just a starting point)
...
| PUT | `/` | admin | Upsert `(company_id, sport_id, item_count)`. Not live — does NOT touch `event_points` |
| DELETE | `/{id}` | admin | Remove a row. Not live — does NOT touch `event_points` |
| POST | `/sports/{sport_id}/recompute` | admin | Bulk-rebuilds `event_points` from `donation_counts` using the bucket rule (synthetic placement 1/2/3). Not currently called by the frontend — ScoringPage saves per-company via the generic `/event-points/award-placement` instead, so admins can override placements before saving; this endpoint remains available for a straight auto-recompute if ever needed |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| POST | `/sports/{sport_id}/recompute` | admin | Bulk-rebuilds `event_points` for a `water_ball_toss` sport from its `matches` (best-of-company rounds survived, ranked, placement scale awarded, ties averaged). Not currently called by the frontend — ScoringPage saves per-company via the generic `/event-points/award-placement` instead, so admins can override placements before saving; this endpoint remains available for a straight auto-recompute if ever needed. |
...
| DonationResultsPage | `/manage/results/donations/:sportId` | Canned Food Drive: per-company running totals — "+ Add" to add to the total, or "Edit total" to set it directly (setting 0 removes the record). Does NOT touch `event_points` — standings are reviewed and saved from Scoring. "Reset All" clears every count and the sport's saved `event_points` together. |
